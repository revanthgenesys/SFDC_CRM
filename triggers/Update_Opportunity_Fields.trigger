trigger Update_Opportunity_Fields on OpportunityLineItem (after delete, after insert, after update) {
    ...
    for (Opportunity opp2 : OppId)
    {
        if(opp2.Forecasted_Flag__c==true){
            ...
            for(OpportunityLineItem ol:[SELECT totalprice,PricebookEntry.Product2.type__c,BigMachines__Origin_Quote__r.Offering_Type__c FROM 
                                        OpportunityLineItem where opportunityid=:opp2.id 
                                        and PricebookEntry.Product2.Forecasted__c = true AND Product_Type__c != 'Cloud' AND GC_Type__c != 'Cloud']){
                                            if(ol.PricebookEntry.Product2.type__c<>'Rental' && (ol.BigMachines__Origin_Quote__r.Offering_Type__c!='PureConnect Subscription'||ol.BigMachines__Origin_Quote__r.Offering_Type__c!='PureEngage Subscription')){
                                            ...
                                            blnUpdatePrice = true;
                                                system.debug(' Term License = ' + term_license);
                                                
                                        } 
            if(blnUpdatePrice){  
                opp2.license_Amount__c = totalprice;
                opp2.term_license__c = term_license;
                system.debug(' License Amount = ' + opp2.license_Amount__c);
            }
        }
        update opp2;    
    }
